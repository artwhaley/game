...
*/
        ...
        private static IEnumerator ImportRoutine(string dtuPath, string fbxPath)
        {
			...
			// Texture imports refresh the Asset Database many times. Unity 6 can
			// invalidate Material object references retained from early iterations,
			// so rebuild the map from the persisted assets before prefab remapping.
			RefreshMaterialMapFromAssets(dtu);
		...
		private static void RefreshMaterialMapFromAssets(DTU dtu)
		{
			foreach (var dtuMaterial in dtu.Materials)
			{
				var key = Utilities.ScrubKey(dtuMaterial.MaterialName);
				var directory = dtu.UseSharedMaterialDir
					? dtu.DTUDir + "/Materials"
					: dtu.GetMaterialDir(dtuMaterial);
				var filename = dtu.UseSharedMaterialDir
					? Utilities.ScrubKey(dtuMaterial.ProductComponentName) + "_" + key + ".mat"
					: key + ".mat";
				var material = AssetDatabase.LoadAssetAtPath<Material>(directory + "/" + filename);
				if (material == null)
				{
					Debug.LogError("DazBridge: persisted material missing before prefab remap: " + directory + "/" + filename);
					continue;
				}
				_map.Map[key] = material;
			}
		}
